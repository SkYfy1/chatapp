...
const ChatList = ({ toggle }) => {
  ...
  const anims = currentUser?.hasOwnProperty("settings")
    ? currentUser?.settings?.animations
    : true;
        ...
        const promises = userChats.map(async (chat) => {
          if (chat.hasOwnProperty("groupMembers")) {
            const members = await Promise.all(
              chat.groupMembers.map(async (user) => {
            ...
            chatId === chat.chatId &&
              console.log("Condition passed, function will be called");
            chatId === chat.chatId &&
              updateGroupInfo({
  ...
  async function handleSelect(chat) {
    ...
    chat.hasOwnProperty("groupName") &&
      (await changeChat(chat.chatId, null, {
        avatar: chat.groupAvatar,
        groupName: chat.groupName,
  ...
  const createGroup = async () => {
    if (appState.groupMembers?.length >= 1) {
      await chatService.createGroupChat(
        [...appState.groupMembers, currentUser.id],
        groupName
      );
            ...
            chats
              ?.filter(
                (el) =>
                  ...
                  el.groupName.toLowerCase().startsWith(filter.toLowerCase())
              )
              .map((chat) =>
                !appState.isMobile ? (
                  <Tooltip
                    key={chat.chatId}
                    style={pointer}
                    text={`${language.settings.tooltip[appState.appLanguage]}${
                      chat?.user?.username || chat?.groupName
                    }`}
                  >
                    <div
                      className="item"
                    ...
                    >
                      <UserListElem
                        chat={chat}
                        type={
                          chat.hasOwnProperty("groupName") ? "group" : "chat"
                        }
                      />
                    </div>
                  </Tooltip>
                  ...
                  >
                    <UserListElem
                      chat={chat}
                      type={chat.hasOwnProperty("groupName") ? "group" : "chat"}
                    />
                  </div>
                )
              )[index]
      ...
      {appState.creatingGroup && (
        <>
          <div className="group">
            ...
            <button onClick={createGroup} className="groupBtn">
              Create Group
            </button>
          </div>
