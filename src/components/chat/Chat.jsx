  ...
  const recordAudioMessage = async () => {
    ...
    try {
      ...
      refic.current.onstop = async (e) => {
        ...
        const blob = new Blob(audioMessageChunks.current, {
  ...
  return (
    <div className={showDetails ? "chat mobile" : "chat"}>
      <div className="top">
        ...
        >
          <img
            src={receiver?.avatar || groupInfo?.avatar || "./avatar.png"}
            alt=""
          />
          <div className="texts">
      ...
      <div ref={reference} className="center">
        ...
        {inView &&
          reference.current.scrollHeight > 900 &&
          chat?.message?.length > 10 && (
            <svg
              ...
              strokeWidth={1.5}
              stroke="currentColor"
              className={showDetails ? "arrowWithDetails" : "arrow"}
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
        ...
        {chat?.messages.length == 0 && (
          <div className="noMessages">
            <div className="alert">Write a message to start a chat!</div>
          </div>
        )}
        {/* Message */}
        {chat?.messages?.map((message) => {
          return (
            <Message
              message={message}
              setShowBig={(value) => setShowBig(value)}
            />
          );
        })}
        <div ref={ref}></div>
      ...
      <div className="bottom">
        {!isMobile ? <Icons>{icons}</Icons> : <Icons>{icon}</Icons>}
        {image.file && (
        ...
        <div
          className="audio"
          onClick={
            !isRecording ? recordAudioMessage : () => refic.current.stop()
          }
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            ...
            strokeWidth={1.5}
            stroke="currentColor"
            className={isRecording ? "rec" : "not-rec"}
          >
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
